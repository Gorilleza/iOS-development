...
{
    char *err_msg = 0;
    printf("1. Insert values to the students table\n");
...
void selectRecords(sqlite3 *db)
{
    ...
    switch(choice)
    {
            ...
            switch (option)
            {
                ...
                case 3:
                    ...
                    if (rc == SQLITE_OK)
                    {
                        ...
                        sqlite3_bind_text(res, idx, str, strlen(str), NULL);
                        PrintSelect(res, 7);
                        sqlite3_finalize(res);
                    }
                    else
        ...
        case 3:
            strcat(sql, "teachers");
            printf("1. Select all teachers\n");
            printf("2. Select teacher by id\n");
            printf("3. Select teachers by last name\n");
            scanf("%d", &option);
            ...
            switch (option)
            {
                ...
                case 2:
                    ...
                    scanf("%s", str);
                    strcat(sql, " where Teacher_id = @id");
                    rc = sqlite3_prepare_v2(db, sql, -1, &res, 0);
                    if (rc == SQLITE_OK)
                    {
                        int idx = sqlite3_bind_parameter_index(res, "@id");
